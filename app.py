...
st.markdown(
    ...
    <style>
        ...
        /* Estilo para que las métricas resalten un poco más */
        [data-testid="stMetricValue"] { font-size: 1.8rem; color: #1b4d3e; }
    </style>
...
def load_data():
    ...
    # Filtro para evitar duplicados (solo sesión completa)
    ...
    with tab1:
        if partido_sel == "- Seleccionar Partido -":
            ...
            if not data_match.empty:
                st.subheader(f"Resumen de datos en {partido_sel}")
                # Mostrar métricas en tarjetas
                m_cols = st.columns(3) # Organizado en 3 columnas para mejor lectura en móvil
                items = list(metricas_dict.items())
                
                for i in range(len(items)):
                    col_id, label = items[i]
                    m_cols[i % 3].metric(label, f"{data_match[col_id].iloc[0]:.1f}")
                
                st.divider()
                # Botón de descarga al final de las métricas
                csv = data_match.to_csv(index=False).encode('utf-8')
                st.download_button(
                    label=f"📥 Descargar reporte CSV de {jugador_sel}",
                    data=csv,
                    file_name=f"{jugador_sel}_{partido_sel}.csv",
                    mime='text/csv'
                )
